...
export function generateFreightPdf(payload: FreightPdfPayload): void {
  ...
  // ---- Footer (sober, name + phone only) ----
  const footerY = H - 36;
  // Subtle separator line
  doc.setDrawColor(226, 232, 240);
  doc.setLineWidth(0.5);
  doc.line(W / 2 - 80, footerY - 14, W / 2 + 80, footerY - 14);

  // Name in bold + phone in muted, on the same centered line
  const name = "By Mr.G";
  const phone = "+228 99584808";
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.setTextColor(15, 23, 42);
  ...
  const totalW = nameW + sepW + phoneW;
  const startX = (W - totalW) / 2;

  doc.setFont("helvetica", "bold");
  doc.setTextColor(15, 23, 42);
  doc.text(name, startX, footerY);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(148, 163, 184);
  doc.text(sep, startX + nameW, footerY);
  doc.setTextColor(71, 85, 105);
  doc.text(phone, startX + nameW + sepW, footerY);
